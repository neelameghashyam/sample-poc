...
// Server-side state
const sortState = ref<DataTableSortState>({ key: 'adoptionDate', direction: 'desc' });
const search = ref('');
...
<template>
  <div class="adopted-view">
    <div class="stat-cards">
      <StatCard
        v-for="card in twpCards"
        :key="card.code"
        :label="card.label"
        :count="card.code === 'all' ? (store.stats.adopted || 0) : (store.stats.twpCounts?.adopted?.[card.code] || 0)"
...
.adopted-view :deep(.data-table__wrapper) {
