...
<template>
  <div class="aborted-view">
    <div class="stat-cards">
      <StatCard
        v-for="card in twpCards"
        :key="card.code"
        :label="card.label"
        :count="card.code === 'all' ? (store.stats.aborted || 0) : (store.stats.twpCounts?.aborted?.[card.code] || 0)"
    ...
    <TestGuidelinesTable
      ...
      :date-column="{ key: 'statusDate', label: 'Aborted Date' }"
...
.aborted-view :deep(.data-table__wrapper) {
