...
const hasActiveMethods = computed(() => form.value.methods.MG || form.value.methods.MS || form.value.methods.VG || form.value.methods.VS || form.value.methods.OTHER);
...
// Dropdown open state management
  ...
  <div class="ch-overlay" @click.self="closeDropdowns">
    <div class="ch-modal">
      ...
      <div class="ch-body">
          ...
          <div class="ch-checkboxes">
            <label class="ch-checkbox-row" @click.prevent="form.asterics = !form.asterics">
              <span class="ch-checkbox" :class="{ 'ch-checkbox--checked': form.asterics }">
                <svg v-if="form.asterics" width="12" height="10" viewBox="0 0 12 10" fill="none">
            ...
            <label class="ch-checkbox-row" @click.prevent="form.grouping = !form.grouping">
              <span class="ch-checkbox" :class="{ 'ch-checkbox--checked': form.grouping }">
                <svg v-if="form.grouping" width="12" height="10" viewBox="0 0 12 10" fill="none">
                  <path d="M1 5L4.5 8.5L11 1" stroke="white" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                </svg>
              </span>
            ...
            <label class="ch-checkbox-row" @click.prevent="form.tq5 = !form.tq5">
              <span class="ch-checkbox" :class="{ 'ch-checkbox--checked': form.tq5 }">
                <svg v-if="form.tq5" width="12" height="10" viewBox="0 0 12 10" fill="none">
                  <path d="M1 5L4.5 8.5L11 1" stroke="white" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                </svg>
              </span>
            ...
            <div class="ch-field">
              ...
              <div class="ch-select-wrap" @click="toggleDropdown('type')">
                ...
                <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
                  <path fill-rule="evenodd" clip-rule="evenodd" d="M1.646 4.646a.5.5 0 0 1 .708 0L8 10.293l5.646-5.647a.5.5 0 0 1 .708.708l-6 6a.5.5 0 0 1-.708 0l-6-6a.5.5 0 0 1 0-.708Z" fill="#727272"/>
                </svg>
                <div v-if="openDropdown === 'type'" class="ch-dropdown">
                  <div
                    v-for="opt in expressionTypes"
                    :key="opt"
                    class="ch-dropdown-item"
                    :class="{ 'ch-dropdown-item--active': form.typeOfExpression === opt }"
                    @click.stop="form.typeOfExpression = opt; openDropdown = null"
                  >{{ opt }}</div>
                </div>
              </div>
            </div>
        ...
        <section class="ch-section">
          ...
          <div class="ch-methods-row">
            <label class="ch-checkbox-row" @click.prevent="form.methods.MG = !form.methods.MG">
              <span class="ch-checkbox" :class="{ 'ch-checkbox--checked': form.methods.MG }">
                <svg v-if="form.methods.MG" width="12" height="10" viewBox="0 0 12 10" fill="none">
                  <path d="M1 5L4.5 8.5L11 1" stroke="white" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                </svg>
              </span>
            ...
            <label class="ch-checkbox-row" @click.prevent="form.methods.MS = !form.methods.MS">
              <span class="ch-checkbox" :class="{ 'ch-checkbox--checked': form.methods.MS }">
                <svg v-if="form.methods.MS" width="12" height="10" viewBox="0 0 12 10" fill="none">
                  <path d="M1 5L4.5 8.5L11 1" stroke="white" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                </svg>
              </span>
            ...
            <label class="ch-checkbox-row" @click.prevent="form.methods.VG = !form.methods.VG">
              <span class="ch-checkbox" :class="{ 'ch-checkbox--checked': form.methods.VG }">
                <svg v-if="form.methods.VG" width="12" height="10" viewBox="0 0 12 10" fill="none">
                  <path d="M1 5L4.5 8.5L11 1" stroke="white" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                </svg>
              </span>
            ...
            <label class="ch-checkbox-row" @click.prevent="form.methods.VS = !form.methods.VS">
              <span class="ch-checkbox" :class="{ 'ch-checkbox--checked': form.methods.VS }">
                <svg v-if="form.methods.VS" width="12" height="10" viewBox="0 0 12 10" fill="none">
                  <path d="M1 5L4.5 8.5L11 1" stroke="white" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                </svg>
              </span>
            ...
            <label class="ch-checkbox-row" @click.prevent="form.methods.OTHER = !form.methods.OTHER">
              <span class="ch-checkbox" :class="{ 'ch-checkbox--checked': form.methods.OTHER }">
                <svg v-if="form.methods.OTHER" width="12" height="10" viewBox="0 0 12 10" fill="none">
                  <path d="M1 5L4.5 8.5L11 1" stroke="white" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                </svg>
              </span>
          ...
          <!-- Plot type selects for MG and MS when checked -->
          <div v-if="form.methods.MG || form.methods.MS" class="ch-plot-selects">
            <div v-if="form.methods.MG" class="ch-select-wrap" @click="toggleDropdown('mg-plot')">
              <span class="ch-select-value" :class="{ 'ch-select-value--filled': form.mgPlotType }">
                {{ form.mgPlotType || 'Select' }}
              </span>
              <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
                <path fill-rule="evenodd" clip-rule="evenodd" d="M1.646 4.646a.5.5 0 0 1 .708 0L8 10.293l5.646-5.647a.5.5 0 0 1 .708.708l-6 6a.5.5 0 0 1-.708 0l-6-6a.5.5 0 0 1 0-.708Z" fill="#727272"/>
              </svg>
            ...
            <div v-if="form.methods.MS" class="ch-select-wrap" @click="toggleDropdown('ms-plot')">
              <span class="ch-select-value" :class="{ 'ch-select-value--filled': form.msPlotType }">
                {{ form.msPlotType || 'Select' }}
              </span>
              <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
                <path fill-rule="evenodd" clip-rule="evenodd" d="M1.646 4.646a.5.5 0 0 1 .708 0L8 10.293l5.646-5.647a.5.5 0 0 1 .708.708l-6 6a.5.5 0 0 1-.708 0l-6-6a.5.5 0 0 1 0-.708Z" fill="#727272"/>
              </svg>
        ...
        <section class="ch-section">
            ...
            <div class="ch-grid-3 ch-grid-3--with-delete">
              ...
              <div class="ch-field">
                ...
                <div class="ch-select-wrap" @click="toggleDropdown('notes-' + state.id)">
                  <span class="ch-select-value" :class="{ 'ch-select-value--filled': state.notes }">
                    {{ state.notes || 'Select notes' }}
                  </span>
                  <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
                    <path fill-rule="evenodd" clip-rule="evenodd" d="M1.646 4.646a.5.5 0 0 1 .708 0L8 10.293l5.646-5.647a.5.5 0 0 1 .708.708l-6 6a.5.5 0 0 1-.708 0l-6-6a.5.5 0 0 1 0-.708Z" fill="#727272"/>
                  </svg>
                ...
                <div class="ch-multiselect" @click.stop="toggleDropdown('varieties-' + state.id)">
                  <div class="ch-multiselect-inner">
                    <span
                      ...
                      :key="v"
                      class="ch-tag"
                      @click.stop="removeVariety(state, v)"
                    >
                      {{ v }} <span class="ch-tag-x">×</span>
                    </span>
                  ...
                  <div v-if="openDropdown === 'varieties-' + state.id" class="ch-dropdown" @click.stop>
                    <div
                      ...
                      :key="v"
                      class="ch-dropdown-item"
                      :class="{ 'ch-dropdown-item--active': state.exampleVarieties.includes(v) }"
                      @click="toggleVariety(state, v)"
                    >{{ v }}</div>
                  </div>
                </div>
              </div>
              <button v-if="form.states.length > 1 || idx > 0" class="ch-delete-state" @click="removeState(state.id)" title="Remove state">
                <svg width="20" height="20" viewBox="0 0 20 20" fill="none">
                  <path d="M2.5 5h15M6.667 5V3.333a1.667 1.667 0 0 1 1.666-1.666h3.334a1.667 1.667 0 0 1 1.666 1.666V5m2.5 0-.833 11.667a1.667 1.667 0 0 1-1.667 1.666H6.667A1.667 1.667 0 0 1 5 16.667L4.167 5h11.666Z" stroke="#727272" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"/>
                </svg>
              </button>
            </div>
          </div>
          ...
          <button class="ch-add-state-btn" @click="addState">
            <svg width="18" height="18" viewBox="0 0 18 18" fill="none">
              <path d="M9 3.75v10.5M3.75 9h10.5" stroke="#1C4240" stroke-width="1.5" stroke-linecap="round"/>
            </svg>
            Add new state
          </button>
        </section>
        ...
        <section class="ch-section">
          ...
          <div class="ch-rte">
            <div class="ch-rte-toolbar">
              <!-- Help -->
              <button class="ch-rte-btn" title="Help">
                <svg width="20" height="20" viewBox="0 0 24 24" fill="none"><g clip-path="url(#a)"><path d="M12 22.5C6.201 22.5 1.5 17.799 1.5 12S6.201 1.5 12 1.5 22.5 6.201 22.5 12 17.799 22.5 12 22.5ZM12 24c6.627 0 12-5.373 12-12S18.627 0 12 0 0 5.373 0 12s5.373 12 12 12Z" fill="#1C4240"/><path d="M7.883 8.679c-.012.205.157.37.361.37h1.237c.207 0 .372-.169.4-.374.133-.985.808-1.702 2.013-1.702 1.028 0 1.97.514 1.97 1.752 0 .952-.561 1.39-1.447 2.057-.91.733-1.709 1.59-1.652 2.98l.005.325c.003.205.17.37.375.37h1.218c.207 0 .375-.168.375-.375v-.158c0-1.076.41-1.39 1.514-2.228.915-.695 1.867-1.467 1.867-3.086 0-2.266-1.914-3.361-4.008-3.361-1.9 0-3.984.886-4.228 3.43ZM10.218 17.323c0 .8.638 1.39 1.514 1.39.914 0 1.542-.59 1.542-1.39 0-.828-.628-1.409-1.542-1.409-.876 0-1.514.581-1.514 1.41Z" fill="#1C4240"/></g><defs><clipPath id="a"><rect width="24" height="24" fill="white"/></clipPath></defs></svg>
              </button>
              <!-- Person -->
              <button class="ch-rte-btn" title="Person">
                <svg width="18" height="20" viewBox="0 0 21 24" fill="none"><path d="M12.75 2.25C12.75 3.493 11.743 4.5 10.5 4.5 9.257 4.5 8.25 3.493 8.25 2.25 8.25 1.007 9.257 0 10.5 0c1.243 0 2.25 1.007 2.25 2.25ZM7.5 8.25 .698 7.587A.797.797 0 0 1 0 6.797C0 6.357.357 6 .797 6h19.406c.44 0 .797.357.797.797 0 .402-.3.741-.698.79L13.5 8.25V13.5l.677 9.63c.04.468-.33.87-.8.87a.866.866 0 0 1-.845-.608l-1.734-8.437a.402.402 0 0 0-.796 0L8.402 23.392A.866.866 0 0 1 7.557 24c-.47 0-.84-.402-.8-.87L7.5 13.5V8.25Z" fill="#1C4240"/></svg>
              </button>
              <!-- Aa -->
              <div class="ch-rte-group">
                <button class="ch-rte-btn" title="Font">
                  <svg width="22" height="20" viewBox="0 0 24 24" fill="none"><path d="M3.366 19.622L4.781 15.418H9.99L11.405 19.622H13.292L8.309 5.625H6.483L1.5 19.622H3.366ZM7.417 7.737l2.09 6.234H5.263L7.365 7.737h.052ZM21.136 18.278H21.188V19.622H22.879V12.187c0-2.266-1.671-3.517-3.968-3.517-2.604 0-3.886 1.374-3.999 3.261h1.661c.103-1.077.892-1.784 2.276-1.784 1.457 0 2.276.78 2.276 2.195v1.097h-2.84c-2.471.01-3.784 1.199-3.784 3.086 0 1.979 1.436 3.27 3.517 3.27 1.59 0 2.574-.645 3.117-1.517Z" fill="#1C4240"/></svg>
                </button>
              ...
              <!-- List -->
              <button class="ch-rte-btn" title="List">
                <svg width="20" height="20" viewBox="0 0 24 24" fill="none"><path fill-rule="evenodd" clip-rule="evenodd" d="M7.5 17.25a.75.75 0 0 1 .75-.75h13.5a.75.75 0 0 1 0 1.5H8.25a.75.75 0 0 1-.75-.75ZM7.5 11.25a.75.75 0 0 1 .75-.75h13.5a.75.75 0 0 1 0 1.5H8.25a.75.75 0 0 1-.75-.75ZM7.5 5.25A.75.75 0 0 1 8.25 4.5h13.5a.75.75 0 0 1 0 1.5H8.25A.75.75 0 0 1 7.5 5.25ZM5.78 3.22a.75.75 0 0 1 0 1.06L3.53 6.53a.75.75 0 0 1-1.06 0L1.72 5.78a.75.75 0 1 1 1.06-1.06l.22.22 1.72-1.72a.75.75 0 0 1 1.06 0ZM5.78 9.22a.75.75 0 0 1 0 1.06l-2.25 2.25a.75.75 0 0 1-1.06 0l-.75-.75a.75.75 0 1 1 1.06-1.06l.22.22 1.72-1.72a.75.75 0 0 1 1.06 0ZM5.78 15.22a.75.75 0 0 1 0 1.06l-2.25 2.25a.75.75 0 0 1-1.06 0l-.75-.75a.75.75 0 1 1 1.06-1.06l.22.22 1.72-1.72a.75.75 0 0 1 1.06 0Z" fill="#1C4240"/></svg>
              </button>
              <!-- Code -->
              <button class="ch-rte-btn" title="Code">
                <svg width="18" height="12" viewBox="0 0 20 12" fill="none"><path d="M6.53 1.28a.75.75 0 0 0-1.06-1.06L.22 5.47a.75.75 0 0 0 0 1.06l5.25 5.25a.75.75 0 0 0 1.06-1.06L1.81 6l4.72-4.72ZM12.97 1.28a.75.75 0 0 1 1.06-1.06l5.25 5.25a.75.75 0 0 1 0 1.06l-5.25 5.25a.75.75 0 0 1-1.06-1.06L17.69 6l-4.72-4.72Z" fill="#1C4240"/></svg>
              </button>
              <!-- Export -->
              <div class="ch-rte-group">
                <button class="ch-rte-btn" title="Export">
                  <svg width="20" height="20" viewBox="0 0 24 24" fill="none"><path d="M13 3V7c0 .553.448 1 1 1h4" stroke="#1C4240" stroke-width="1.2" stroke-linecap="round" stroke-linejoin="round"/><path d="M10.5 21H6a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h7l5 5v8M13 19h7M20 19l-3-3M20 19l-3 3" stroke="#1C4240" stroke-width="1.2" stroke-linecap="round" stroke-linejoin="round"/></svg>
                </button>
                <span class="ch-rte-caret"><svg width="14" height="14" viewBox="0 0 16 16" fill="none"><path fill-rule="evenodd" clip-rule="evenodd" d="M1.646 4.646a.5.5 0 0 1 .708 0L8 10.293l5.646-5.647a.5.5 0 0 1 .708.708l-6 6a.5.5 0 0 1-.708 0l-6-6a.5.5 0 0 1 0-.708Z" fill="#727272"/></svg></span>
              </div>
              <!-- Paint -->
              <button class="ch-rte-btn" title="Paint">
                <svg width="20" height="20" viewBox="0 0 24 24" fill="none"><g clip-path="url(#b)"><path d="M0 3a3 3 0 0 1 3-3h13.5a3 3 0 0 1 3 3v3a3 3 0 0 1-3 3H3a3 3 0 0 1-3-3V3ZM7.5 16.5A1.5 1.5 0 0 1 9 15v-.75A2.25 2.25 0 0 1 11.25 12H18a3 3 0 0 0 3-3V3.258A4.5 4.5 0 0 1 24 7.5V9c0 2.485-2.016 4.5-4.5 4.5H12.75A.75.75 0 0 0 12 14.25V15a1.5 1.5 0 0 1 1.5 1.5v6A1.5 1.5 0 0 1 12 24H9A1.5 1.5 0 0 1 7.5 22.5v-6Z" fill="#1C4240"/></g><defs><clipPath id="b"><rect width="24" height="24" fill="white"/></clipPath></defs></svg>
              </button>
              <!-- Link -->
              <button class="ch-rte-btn" title="Link">
                <svg width="20" height="20" viewBox="0 0 24 24" fill="none"><path d="M10 13a5 5 0 0 0 7.54.54l3-3a5 5 0 0 0-7.07-7.07l-1.72 1.71" stroke="#1C4240" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/><path d="M14 11a5 5 0 0 0-7.54-.54l-3 3a5 5 0 0 0 7.07 7.07l1.71-1.71" stroke="#1C4240" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg>
              </button>
              <!-- Highlight -->
              <button class="ch-rte-btn" title="Highlight">
                <svg width="20" height="20" viewBox="0 0 24 24" fill="none"><g clip-path="url(#c)"><path fill-rule="evenodd" clip-rule="evenodd" d="M16.644.967c1.179-1.121 3.037-1.097 4.187.054l2.148 2.148c1.15 1.15 1.174 3.008.054 4.187l-.62.652L10.308 21.5a.75.75 0 0 1-.558.25H5.25a.75.75 0 0 1-.607-.316L4.28 22.28a.75.75 0 0 1-1.06 0l-2.5-2.5a.75.75 0 0 1 .22-1.22l2.118-2.118A.75.75 0 0 1 2.25 16.25v-4.5a.75.75 0 0 1 .22-.53L15.99 1.586l.653-.62ZM16.472 3.17L4.09 14.279l5.63 5.63L20.83 7.528 16.472 3.17ZM21.849 6.425l.096-.098c.56-.59.548-1.518-.027-2.093L19.768 2.08c-.575-.575-1.504-.563-2.094.054l-.1.098 4.274 4.274ZM7.94 20.25H3.75l-1.19-1.19 2.188-2.188L7.94 20.25Z" fill="#1C4240"/></g><defs><clipPath id="c"><rect width="24" height="24" fill="white"/></clipPath></defs></svg>
              </button>
              <!-- Table -->
              <div class="ch-rte-group">
                <button class="ch-rte-btn" title="Table">
                  <svg width="20" height="20" viewBox="0 0 24 24" fill="none"><g clip-path="url(#d)"><path d="M0 3a3 3 0 0 1 3-3h18a3 3 0 0 1 3 3v18a3 3 0 0 1-3 3H3a3 3 0 0 1-3-3V3Zm22.5 3H16.5V10.5h6V6Zm0 6H16.5V16.5h6V12Zm0 6H16.5V22.5H21a1.5 1.5 0 0 0 1.5-1.5V18ZM15 22.5V18H9v4.5H15Zm-7.5 0V18H1.5V21A1.5 1.5 0 0 0 3 22.5h4.5Zm-6-7.5h6V10.5H1.5V15Zm0-6H7.5V4.5H3a1.5 1.5 0 0 0-1.5 1.5V9Zm7.5-4.5V9H15V4.5H9ZM15 10.5H9V15h6V10.5Z" fill="#1C4240"/></g><defs><clipPath id="d"><rect width="24" height="24" fill="white"/></clipPath></defs></svg>
                </button>
                <span class="ch-rte-caret"><svg width="14" height="14" viewBox="0 0 16 16" fill="none"><path fill-rule="evenodd" clip-rule="evenodd" d="M1.646 4.646a.5.5 0 0 1 .708 0L8 10.293l5.646-5.647a.5.5 0 0 1 .708.708l-6 6a.5.5 0 0 1-.708 0l-6-6a.5.5 0 0 1 0-.708Z" fill="#727272"/></svg></span>
              </div>
              <!-- Image -->
              <button class="ch-rte-btn" title="Image">
                <svg width="20" height="18" viewBox="0 0 24 21" fill="none"><path d="M9 6.75A2.25 2.25 0 1 1 4.5 6.75 2.25 2.25 0 0 1 9 6.75Z" fill="#1C4240"/><path d="M3 0a3 3 0 0 0-3 3v15a3 3 0 0 0 3 3h18a3 3 0 0 0 3-3V3a3 3 0 0 0-3-3H3Zm18 1.5A1.5 1.5 0 0 1 22.5 3v9.75l-5.665-2.921a.75.75 0 0 0-.804.073l-5.565 5.565-3.988-2.659a.75.75 0 0 0-.926.073L1.5 16.5V3A1.5 1.5 0 0 1 3 1.5H21Z" fill="#1C4240"/></svg>
              </button>
            </div>
            <div class="ch-rte-body">
              <textarea
                v-model="textareaValue"
                class="ch-rte-textarea"
                :placeholder="mode === 'edit' ? 'Insert the text area' : 'Add explanation here'"
              ></textarea>
              <span class="ch-rte-resize">
                <svg width="12" height="12" viewBox="0 0 13 13" fill="none"><path d="M11.099 12.356 12.356 11.099c.346-.346.389-.869.095-1.163-.294-.294-.817-.251-1.163.095L10.031 11.288c-.346.346-.388.869-.095 1.163.294.294.817.251 1.163-.095ZM.195 11.509c.294.294.817.251 1.163-.095L11.415 1.358c.346-.346.388-.869.094-1.163-.293-.294-.816-.251-1.162.095L.29 10.346c-.346.346-.388.869-.095 1.163ZM5.914 12.199l6.285-6.285c.346-.346.388-.869.095-1.163-.294-.294-.817-.251-1.163.095L4.846 11.131c-.346.346-.388.869-.095 1.163.294.294.817.251 1.163-.095Z" fill="#1C4240"/></svg>
              </span>
            </div>
          </div>
        </section>
      ...
      <div class="ch-footer">
        ...
        <button class="ch-save-btn" @click="handleSave">
          ...
          <svg v-else width="16" height="16" viewBox="0 0 18 18" fill="none">
            <circle cx="9" cy="9" r="7.5" stroke="#DADE14" stroke-width="1.3"/>
            <path d="M9 6v6M6 9h6" stroke="#DADE14" stroke-width="1.5" stroke-linecap="round"/>
          </svg>
...
.ch-save-btn:hover { opacity: 0.88; }
</style>